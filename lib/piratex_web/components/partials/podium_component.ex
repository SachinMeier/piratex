  ...
  def podium(assigns) do
    ...
    <div class="hidden lg:grid lg:grid-cols-3 gap-2">
      <div class="my-2 col-1 pt-16">
        <%= if @team_ct > 2 do %>
          <.podium_team
            team={Enum.at(@ranked_teams, 2) |> elem(1)}
            players={@players}
            rank={Enum.at(@ranked_teams, 2) |> elem(0)}
            podium={true}
          />
        <% else %>
          &nbsp;
      ...
      <div class="my-2 col-2">
        <.podium_team
          team={Enum.at(@ranked_teams, 0) |> elem(1)}
          players={@players}
          rank={Enum.at(@ranked_teams, 0) |> elem(0)}
          podium={true}
        />
      </div>
      <div class="my-2 col-3 pt-8">
        <%= if @team_ct > 1 do %>
          <.podium_team
            team={Enum.at(@ranked_teams, 1) |> elem(1)}
            players={@players}
            rank={Enum.at(@ranked_teams, 1) |> elem(0)}
            podium={true}
          />
        <% end %>
      </div>
