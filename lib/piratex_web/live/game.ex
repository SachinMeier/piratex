defmodule PiratexWeb.Live.GameLive do
  ...
  def mount(_params, _session, socket) do
    ...
    IO.inspect(game_state.challenges, label: "challenges")
  ...
  def render_finished(assigns) do
    ...
    <div class="flex flex-col w-full mx-auto items-center">
      ...
      <div class="flex flex-col">
        <%= for {rank, %{name: player_name, score: score, words: words}} <- @ranked_players do %>
          <div class="my-2">
            <.render_player_word_area
              player_name={"#{rank}. #{player_name} (#{score})"}
              words={words}
            />
          </div>
        <% end %>
    ...
    <div class="flex flex-col">
      ...
      <div id="board" class="flex flex-col md:flex-row justify-between w-full mt-8">
        <div id="board_players" class="flex flex-wrap gap-4">
          <%= for %{name: player_name, words: words} <- @game_state.players do %>
            <.render_player_word_area
  ...
  defp render_center(assigns) do
    ...
    ~H"""
    <div id="board_center" class="flex flex-wrap gap-x-4 w-full max-h-52 overflow-y-auto overscroll-contain no-scrollbar rounded-md p-4">
  ...
  defp render_player_word_area(assigns) do
    ~H"""
    <div id={"board_player_#{@player_name}"} class="flex flex-col min-w-48 rounded-md border-2 border-black dark:border-white min-h-48">
  ...
  defp render_player_action_area(assigns) do
    ...
    <div id="actions_area" class="flex flex-col">
      <div class="flex flex-row md:flex-col gap-4">
        <.form for={@word_form}  phx-submit="submit_new_word" phx-change="change" class="flex flex-row">
          <.ps_text_input
            id="new_word_input"
            name="word"
            form={@word_form}
        ...
        <div class="w-full mx-auto">
          <.ps_button class="w-full max-w-xs mx-auto" phx_click="flip_letter" phx_disable_with="Flipping..." disabled={!@is_turn || @paused}>
            <%= if @is_turn and @game_state.letter_pool != [] do %>
              FLIP
            <% else %>
  ...
  def handle_event("submit_new_word", %{"word" => word}, socket) do
    socket =
