defmodule Piratex.ChallengeService do
  ...
  defmodule Challenge do
    ...
    def start_challenge_timeout(challenge_id) do
      Process.send_after(
        self(),
        {:challenge_timeout, challenge_id},
        Config.challenge_timeout_ms()
      )
    end
  ...
  """
  @spec handle_word_challenge(Game.t(), String.t(), String.t()) ::
          Game.t() | {:error, challenge_error()}
  ...
  @type challenge_vote_error ::
          :challenge_not_found | :player_not_found | :already_voted | :unknown_error
  ...
  def handle_challenge_vote(%{players: _players} = state, player_token, challenge_id, vote) do
    with {_, {challenge_idx, challenge = %Challenge{}}} <-
         ...
         {_, player = %Player{status: :playing}} <-
  ...
  defp challenge_resolution_message(%WordSteal{victim_word: nil, thief_word: thief_word}, result) do
    "Challenge resolved: #{String.upcase(thief_word)} is #{verdict(result)}."
  end

  defp challenge_resolution_message(
         %WordSteal{victim_word: victim_word, thief_word: thief_word},
         result
       ) do
    "Challenge resolved: #{String.upcase(victim_word)} to #{String.upcase(thief_word)} is #{verdict(result)}."
  end

  defp verdict(:valid), do: "VALID"
